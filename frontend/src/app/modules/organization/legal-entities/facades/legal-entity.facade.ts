...
export class LegalEntityFacade {
  private readonly api = inject(LegalEntityApiService);
  private readonly errorMapper = inject(ErpErrorMapperService);
  private readonly destroyRef = inject(DestroyRef);

  // ─── List State ───────────────────────────────────────────────
  private readonly entitiesSignal = signal<LegalEntityListItemDto[]>([]);
  ...
  private readonly lastSearchRequestSignal = signal<SearchRequest>({
    filters: [],
    sorts: [{ field: 'legalEntityCode', direction: 'ASC' }],
    page: 0,
    size: 20
  });
  ...
  private readonly currentEntitySignal = signal<LegalEntityDto | null>(null);
  private readonly usageInfoSignal = signal<LegalEntityUsageDto | null>(null);
  ...
  applyGridStateAndLoad(params: {
    ...
    const request: SearchRequest = {
      ...
      sorts: params.sortBy ? [{ field: params.sortBy, direction: params.sortDir ?? 'ASC' }]
                           : [{ field: 'legalEntityCode', direction: 'ASC' }],
      page: params.page,
      size: params.size
    };
  ...
  getById(id: number, onSuccess?: (entity: LegalEntityDto) => void): void {
    this.loadingSignal.set(true);
  ...
  create(request: CreateLegalEntityRequest, onSuccess?: (entity: LegalEntityDto) => void): void {
  ...
  update(id: number, request: UpdateLegalEntityRequest, onSuccess?: (entity: LegalEntityDto) => void): void {
    this.savingSignal.set(true);
    this.saveErrorSignal.set(null);
  ...
  getUsageInfo(id: number, onSuccess?: (usage: LegalEntityUsageDto) => void): void {
