...
import { ConfirmActionDeps, confirmDeactivateRegion } from '../../helpers/region-confirm-actions';
import {
  createRegionFilterOptions,
  createRegionColumnDefs,
  createRegionGridOptions,
  ERP_DEFAULT_COL_DEF
} from './region-grid.config';
...
export class RegionSearchComponent extends ErpListComponent implements OnInit {
  ...
  get rowData(): RegionListItemDto[] { return this.facade.entities(); }
  ...
  private rebuildGridConfig(): void {
    const filterOpts = createRegionFilterOptions(this.translate, this.statusOptions);
    this.availableFields = filterOpts.fields;
    this.availableOperators = filterOpts.operators;

    this.columnDefs = createRegionColumnDefs(this.translate, this.zone, {
      onEdit: (region) => this.navigateToEdit(region),
      onDeactivate: (region) => this.deactivateRegion(region)
    });

    const gridCfg = createRegionGridOptions(this.translate);
  ...
  navigateToCreate(): void { this.router.navigate(['/organization/regions/create']); }

  navigateToEdit(region: RegionListItemDto): void {
    this.router.navigate(['/organization/regions/edit', region.regionPk]);
  }

  deactivateRegion(region: RegionListItemDto): void {
    confirmDeactivateRegion(this.confirmDeps, region, () => this.reload());
  }
