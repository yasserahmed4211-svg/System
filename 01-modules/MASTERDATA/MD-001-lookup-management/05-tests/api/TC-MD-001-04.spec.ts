...
test.describe('G4 — MdMasterLookup: Toggle Active', () => {
  ...
  // ── TC-028 ─────────────────────────────────────────────────────────────────
  // ⚠️ BUG-04: Redis DOWN → /api/lookups/{key} returns 500; cache eviction cannot be verified
  test('[TC-028] [BUG-04] Cache evicted after toggle — GET /api/lookups/{key} should be live; KNOWN Redis DOWN → 500', async () => {
    ...
    await test.step('Toggle then hit consumption endpoint', async () => {
      ...
      // Check consumption — expected: 200 (key exists, master inactive → empty list)
      // Actual (BUG-04): 500 (Redis required but DOWN)
      ...
      expect(cacheResp.status(), '[BUG-04] Redis DOWN; consumption endpoint returns 500').toBe(200);
    });
  });
});
