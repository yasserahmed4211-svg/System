/**
 * DB Validation Helper — ORG-001 Organization Module
 *
 * Oracle MCP direct connection is unavailable (JDBC driver issue in MCP).
...
/**
 * Validate that a record is no longer active (deactivated).
 */
export async function validateDeactivated(
  ctx: APIRequestContext,
  endpoint: string,
  id: number | string,
  headers: Record<string, string>,
  ...
  const data = body?.data ?? {};
  return {
    validated: resp.status() === 200 && data.isActive === false,
    evidence: { httpStatus: resp.status(), isActive: data.isActive },
  };
}
